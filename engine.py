...
class RepairEngine:
    ...
    def __init__(self, rules_path="pat_rules.md"):
        ...
        """
        # Change API Key as needed for different models
        api_key = os.getenv("GEMINI_API_KEY")
        if not api_key:
            raise ValueError("API_KEY missing from .env")

        # Change base_url as needed for different models
        self.client = OpenAI(
            api_key=api_key,
    ...
    def request_repair(
        self, full_context, error_log, target_assertion="", other_assertions=None
    ):
                ...
                messages=[
                    {
                        "role": "system",
                        "content": f"""
You are an expert in PAT (Process Analysis Toolkit) and CSP# formal model repair.

MANDATORY PAT SYNTAX RULES:
{self.mandatory_syntax}
""",
                    },
            ...
            if "429" in str(e):
                ...
                return self.request_repair(
                    full_context, error_log, target_assertion, other_assertions
                )
    ...
    def _looks_like_starvation_liveness(self, target_assertion, error_log):
        if "[]<>" not in target_assertion:
            return False
        return (
            "Starvation" in error_log
            or "SCC" in error_log
            or re.search(r"\)\s*\*", error_log)
        )
